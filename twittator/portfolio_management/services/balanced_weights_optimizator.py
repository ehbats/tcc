...
class BalancedWeightsOptimizator(Optimizator):
    """
    ...
    stocks which have a positive predicted return in n days and balances
    these weights among these stocks. For example: if there are three stocks,
    with expected returns equal to 3%, 5% and 2%, the model will put 30% on the first
    stock, 50% on the second stock and 2% on the third stock. This strategy
    may be very risky, because it will bet a lot on high expected returns, ignoring 
    the variance of tha stock's price series.
    The strategy keeps
    ...
    *The dataframes inside these lists will be merged for the analysis.*
    ...
    def get_row_returns(
        ...
        if not reset:
            ...
            total_return = sum([stock_return for stock_return in expected_returns_list if stock_return > 0])

            self.weights = [stock_return/total_return if stock_return > 0 else 0 for stock_return in expected_returns_list]
